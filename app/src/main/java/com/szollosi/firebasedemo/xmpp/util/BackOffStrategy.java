...
public class BackOffStrategy {
  ...
  private static final String TAG = BackOffStrategy.class.getName();
  ...
  public void errorOccured2() throws Exception {
    ...
    if (!shouldRetry()) {
      throw new Exception(
          "Retry Failed: Total of attempts: " + numberOfRetries + ". Total waited time: " +
              timeToWait + "ms.");
    }
    waitUntilNextTry();
    timeToWait *= 2;
  ...
  public void errorOccured() {
    ...
    if (!shouldRetry()) {
      Log.i(TAG,
          "Retry Failed: Total of attempts: " + numberOfRetries + ". Total waited time: {} ms." +
              timeToWait);
    }
    waitUntilNextTry();
    timeToWait *= 2;
  ...
  private void waitUntilNextTry() {
    try {
      ...
      Log.i(TAG,
          "[waitUntilNextTry] Error waiting until next try for the backoff strategy. Error: " +
              e.getMessage());
    }
  }
